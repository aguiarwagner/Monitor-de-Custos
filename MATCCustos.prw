...
Static Function FPanel04(oPanel)
	...
	*/
	...
	oBtProces := TButton():New( 013, 545, "Processar",oPanel,{||MATCPCalc(.F.)}, 40,12,,,.F.,.T.,.F.,,.F.,,,.F. )
...
Static Function MATCPrDiv()
	...
	oBtProces := TButton():New( 008, 260, "Processar",oList,{||MATCPCalc(.T.)}, 40,12,,,.F.,.T.,.F.,,.F.,,,.F. )
...
Static Function MATCPCalc(lRot)

	Local cQuery      := ""
	Local cData       := "s"
	Local bLine       := ""
	Local QryTSD1An, QryTSD2An, QryTSD3An, QryTSD3AnP, QrySB9An, QrySB2An, QryTempDB, QrySB2FIL, QrySB2 := ""
	Local aFields     := {}
	Local aSaldIni    := {}
	Local lRet		  := .T.
	Local aCusOp      := {}
	...
	Local nValFin     := 0
	Local nCont       := 0
	Local aTamSD3     := TamSx3('B2_VATU1')
	//Local oSaldIni    As object
	//Local oData       As object

	Default oListBox := Nil
	Default lRot     := .F. //Indica a rotina que está sendo processada
	// Zera variáveis da tela
	nQtSB9      := 0
	nVLSB9      := 0
	nVlOpSB9	:= 0
	...
	If lRet
		...
		oTempTable := FWTemporaryTable():New( "MATCTmpAn" ) //oTempTable:Delete()
...
FOR nCont := 1 TO LEN(aArrSc2)
	IF LEN(aSd3Prod) > 0
		...
		nCusOP := 0
	ELSE
		//OPs que possuem requisição porém não possuem apontamento
		cOp     := aArrSc2[nCont][1]
